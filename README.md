...
## Description
The ```geotools``` package is an R library designed to match spatial georeferenced data with the administrative boundaries of Italy. In particular, it enables the computation of statistics for space-time variables (e.g., meteorological and air quality data) whose values are available on different spatial heterogeneous grids at the polygonal scale (e.g., municipal or regional).
